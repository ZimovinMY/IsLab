...
В папке requests находится скрипт req.py, который отправляет запросы к основному сервису предсказаний. Для формирования запросов скрипт случайным образом генерирует значения полей входных данных, а также выполняет их отправку через случайные промежутки времени.
## Prometheus
Prometheus регулярно собирает метрики с сервиса предсказаний, чтобы их можно было визуализировать на графиках. Для обработки данных используются запросы на языке PromQL. 
Конфигурационный файл prometheus.yml определяет цели для опроса, частоту сбора данных и параметры опроса.
- График предсказаний модели по интервалам
![image](https://github.com/user-attachments/assets/698d0993-1dca-46f6-b288-81dd01f4754b)
- График частоты запросов к основному сервису в минуту
![image](https://github.com/user-attachments/assets/38074dab-b35e-4229-8509-0887b56ac20f)
- График количества запросов к сервису с кодами ошибок 4** и 5**
![image](https://github.com/user-attachments/assets/9e2675af-4071-4fe4-ac33-c3aaf4af280e)
## Grafana
Grafana предоставляет возможность подключить Prometheus в качестве источника данных. Пользователь настраивает панели для визуализации метрик в виде графиков, таблиц и других форматов. Для каждой панели можно выбрать собственный запрос к данным и подходящий тип визуализации.
### Grafaana Dashboard
![image](https://github.com/user-attachments/assets/19b4479c-a5b8-4fe0-a254-69d6f32d6e91)
#### Bucket Predictions
График показывает распределение предсказаний модели по различным интервалам. Относится к прикладному уровеню мониторинга.
#### Requests Total
График показывает общее количество HTTP-запросов к API /api/prediction, с разбивкой по статусам (2xx, 4xx, 5xx). Относится к прикладному уровеню мониторинга.
#### CPU Seconds Total
График показывает общее время работы CPU для процесса price-predict:8000. Относится к инфраструктурному уровеню мониторинга.
#### Memory
График отображает объем используемой памяти в байтах для процесса price-predict:8000. Относится к инфраструктурному уровеню мониторинга.
## Веб-интерфейс сервисов
```
http://localhost:8000 - сервис предсказаний
